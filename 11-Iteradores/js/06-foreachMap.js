...
// el map te crea un nuevo arreglo,
// si solo deseas recorrer los elementos utiliza el Foreach,
// si requieres crear un nuevo arreglo, sin duda el map sera mejor...
const nuevoArray = carrito.map( function(p) {
    return  `Articulo: ${ p.id } Precio: ${p.producto} `;
});

console.log( 'console.log(nuevoArray);', nuevoArray);
const nuevoArray2 = carrito.map( function(p) {
    return  { Uid: p.id, pro: p.producto };
});

console.log( 'nuevoArray2', nuevoArray2);
